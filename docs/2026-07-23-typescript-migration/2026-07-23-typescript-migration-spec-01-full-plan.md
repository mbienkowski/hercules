...
| 15 | ✅ done | `feat!: cut the build, ci and release pipeline over to typescript` | Genuinely just the two Makefile lines the plan predicted: `build`/`build-check` now run `node .ts-out/bin/cli.mjs` (with a `compile` prerequisite, matching every other TS-consuming target) instead of `python -m scripts.build.cli`. Every consumer (`build_gates.sh`, `test-py`'s `build-check` dependency, `release.yml`'s `make build` step) already called back through `make` rather than hardcoding the compiler (few-shot #2, paid off from commit 13), so nothing else needed editing. A pre-flip grep dry-check (commit 16's own eventual gate, run early as the plan's residual-risk mitigation suggests) confirmed every remaining `scripts.build`/`scripts/build` reference is either the still-live, not-yet-ported `tests/build/*.py` files (import the modules directly, unaffected by what `make build` points to) or prose/config that's explicitly commit 16's concern (`pyproject.toml`'s `[tool.mutmut]`, CoC/CONTRIBUTING prose, the mutation job's CI comment) — nothing was missed | `make build-check`/`make ci-build` green against the REAL committed `dist/` (last generated by the old Python-driven build) — the strongest available proof, since it means the production Makefile target, now pointing at TypeScript, reproduces committed history byte-for-byte, not just a fresh self-consistent tree; `git diff dist/` empty throughout; `tsc -b` clean; `vitest run` 1006/1 and Python suite 372/16 both unchanged; `make parity` still 181/181 + full-tree leg (the parity harness's own engines are untouched by this commit — it drives both compilers directly, never through `make build`) |
| 16 | ✅ done (mutation kill-rate deferred) | `chore(build): delete the python compiler and split the mutation gates` | Removes `scripts/build/` (11 files). Before deletion, every remaining Python compiler test that imported it (20 files under `tests/build/`, 154 tests total via `pytest --collect-only`) was ported to `tests-ts/build/*.spec.ts` by three parallel agents (smoke specs / ecosystem build+serialize / core compiler primitives), each independently verified against the real count and cross-checked by the orchestrating session (154 matched exactly); Python originals + the now-empty `tests/build/` deleted in this commit. Rescopes `[tool.mutmut]` to `src/hooks/` **in the same commit** (lesson 11) — `runner`/`tests_dir` narrowed from `tests/,tests-python/` to `tests-python/` alone, since nothing in the main `tests/` tree exercises hook mutation behavior; drops `tiktoken`/`pyyaml`/`tomli`/`pytest-mock` from the Python dev deps (verified zero remaining importers of each). Splits CI's single `mutation` job into parallel `mutation-py` (Python-only setup, `needs`/`if` unchanged) and `mutation-ts` (Node-only setup) — job id, display name, and `make` target now the same string for each (few-shot #12), and each job's own setup narrowed to the ONE runtime it actually needs (a real simplification, not just a mechanical split). Also retires the now-purposeless dual-run parity harness (`scripts/ci/{parity_run.py,parity.sh,token_parity.py,parity_tokens.sh}` — both `.py` files were self-documented "Temporary — deleted with the Python compiler"; their TS counterparts `scripts-ts/bin/{parityRun,tokenParity}.mts` and the 181 fixtures under `tests/testdata/parity/` retired alongside, since there is no second engine left to diff against) and the two now-dead `*ForFixture` test-support functions the harness was the only caller of (`genSerialize.mts`/`genExtras.mts`). Wires the 6 ecosystems' `smoke.test` descriptor fields + `run_smoke.sh` + `make test-smoke` from the deleted `.py` smoke specs to their new `.spec.ts` paths (Vitest transforms TS on the fly — no separate compile step needed, unlike the `node .ts-out/...` entry points elsewhere). Fixes 3 real gaps found during this commit's own audit, none introduced by it: `.githooks/pre-commit` still hardcoded `python3 -m scripts.build.cli` directly (bypassing `make build`, a commit-15 gap — few-shot #2 violation); `scripts/ci/release_commit.sh` still shelled out to the just-deleted `scripts.build.version_targets` (now reads `VERSION_TARGETS` from the `prepare` job's downloaded `.ts-out/` artifact via `node --input-type=module`, keeping `npm ci` out of the privileged job per commit 13's design); `tests/test_collection_integrity.py`'s positive-companion check asserted `tests/build/` exists (removed — its premise is now false by design, the general folder-name guard it paired with stays). Every dangling `scripts/build/<file>.py` doc/comment reference fixed to point at the TS equivalent (CoC, CONTRIBUTING, the OpenCode template's own generated-by header, `annotate_smoke.sh`'s already-stale-before-this-migration `manifests.py` pointer); the CoC's broader Python-flavored prose (§ Testing, § Adding an ecosystem) deliberately left for commit 17, which owns that rewrite explicitly | `grep -rn 'scripts\.build\|scripts/build/'` across live (non-historical-comment) files returns zero; `tsc -b` clean; `vitest run` 1108 passed/12 skipped (expected — live-CLI tests for binaries not installed here); Python suite 233 passed (down from 372 — exact bottom-up reconciliation: 223 hooks-island unchanged since commit 12, + 8 `test_mutation_gate.py` + 1 `test_no_retired_path_refs.py` + 1 `test_collection_integrity.py`); `make ci-build` green, `dist/` byte-identical (only `opencode/plugin.js`'s generated-by header changed, from the template fix); `pip install -e ".[dev]"` config verified as valid TOML with the trimmed dep list. **Mutation kill-rate verification itself (`make mutation-py`/`make mutation-ts` actually reporting ≥90%) is explicitly deferred to the final post-commit-17 pass**, per the user's standing instruction earlier in this migration — this commit lands the structural split (config + CI jobs) correctly but does not run a full mutation pass |
| 17 | ✅ done | `docs: describe the two-runtime world` | `CODE_OF_CONDUCT.md` § Testing rewritten: "One language, one runner: Python" → two runtimes/two runners/one bar, describing the real parallel `mutation-py`/`mutation-ts` CI split from commit 16 and js-tiktoken's bundled-ranks (no cache dir) tokenizer; § Adding an ecosystem made language-neutral (`cli.build_target`→`buildTarget`, `pyproject.toml`→`package.json` for the version token per commit 14's flip, `tests/build/`→`tests-ts/build/`, "mutation-covered Python behaviors"→"mutation-covered behaviors" — file paths corrected to their real TS location, generic-principle language de-Pythoned, per-file locations left accurate rather than vague); § Invariants' version-single-source bullet (already correct from commit 14) and its "Enforced by `tests/`" header (now `tests-ts/`+`tests-python/`) updated. § Hooks' "Stdlib-only Python, no shebang" kept **verbatim**, with a new `LOCKED` blockquote note at the section's top explicitly marking `src/hooks/` as a permanent exception, not a migration-in-progress state — so a future contributor doesn't read "everything migrates to TS" into the one deliberate carve-out. `src/content/persona.md`'s condensed Testing summary (compiled into every ecosystem's persona/instructions file) rewritten to match, kept to the same length to respect the instruction/token budget gates it's itself subject to. `CONTRIBUTING.md`'s Quick Start (`make install` now explicitly named as both-runtime) and "no per-ecosystem Python" → "no per-ecosystem code" | `grep -rn 'All checks live in Python' src/ dist/` empty (the plan's own literal criterion); rebuild shows exactly the expected diff — every ecosystem's persona-derived file's `## Testing` section, nothing else; `tsc -b` clean; `vitest run` 1108/12 unchanged (no instruction-budget regression from the persona.md edit); Python suite 233 unchanged; `make ci-build` green once committed; final repo-wide sweep for `scripts/build/`, `scripts.build`, and the old Testing-section phrasing finds only the 4 already-reviewed historical/past-tense comments from commit 16, nothing new |
