...
| 9 | ⬜ | `test(build): port the compiler test suite to vitest` | 40 files → `tests-ts/build/*.spec.ts`. Adds the positive-path test nobody had: a synthetic 7th descriptor built only from existing vocabulary compiles with zero `.mts` changes | `make test-ts` ≥90% branch; `make mutation-ts` ≥90% over the currently-gated modules; per-spec inject/revert recorded |
