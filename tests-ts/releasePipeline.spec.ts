...
describe('the build-output tracked guard', () => {
  it('CI runs the build gates, whose script checks dist/ is tracked', () => {
    const gatesSh = readRepoFile('scripts', 'ci', 'build_gates.sh');
...
describe('CI jobs that run compiled TypeScript scripts install the toolchain first', () => {
  // The build job emits the smoke matrix (scripts-ts/ci/smokeMatrix.mts, which imports the
  // zod-validated descriptor module) and validate checks the package (scripts-ts/ci/validatePackage.
...
describe('the release pipeline', () => {
  ...
  it('rebuilds and commits the output after bumping the version, in order', () => {
    const commitSh = readRepoFile('scripts', 'ci', 'release_commit.sh');
  ...
  it('acts only on the CI-validated commit', () => {
    const verifySh = readRepoFile('scripts', 'ci', 'release_verify_checkout.sh');
