...
describe('the shared kill-rate thresholds', () => {
  it('holds this project to a 90% kill rate, warning from 95%', () => {
    ...
    // scripts/mutation-gate.json and this fails.
    //
    // It matters more than it looks. scripts/check_mutation_gate.py reads the same file at import
    // time, so today the Python suite would also catch a change — but that gate narrows to
    // src/hooks/ later in this migration, and when it does this becomes the only end still pinned.
  ...
  it('resolves and parses the shared file rather than carrying its own copy', () => {
    ...
    const shared = readRepoJson<{ gate: number; warn: number }>('scripts', 'mutation-gate.json');
