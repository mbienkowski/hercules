...
import {
  ...
  parseDescriptor,
} from '../../scripts-ts/build/descriptor.mjs';
import { SmokeMatrixError, buildMatrix, main } from '../../scripts-ts/ci/smokeMatrix.mjs';
...
function fakeDescriptor(name: string): EcosystemDescriptor {
  return parseDescriptor(name, minimal({ name, smoke: { cli: name, test: 'tests-ts/x.spec.ts' } }));
}
