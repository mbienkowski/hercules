...
// A SEPARATE spec file, deliberately — see tests-ts/build/descriptorSort.spec.ts's own header
...
const { discover } = await import('../../scripts-ts/build/descriptor.mjs');
const { roleEntries } = await import('../../scripts-ts/build/genExtras.mjs');
const { ECOSYSTEMS } = await import('../support/descriptorFixtures');
const { repoRoot } = await import('../support/repo');
