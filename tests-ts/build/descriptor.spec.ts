...
describe('accepting the maximal valid shape', () => {
  it('parses every field generator, route kind, and template value kind into the EXACT expected shape', () => {
    ...
    // output live in tests-ts/testdata/descriptor/maximal-valid-shape.*.json (CoC's 20-line test /
    // 500-line file caps — this fixture inline was 250+ lines by itself) and were independently
    // proven correct against the Python original by the 137 parity fixtures under
    // tests/testdata/parity/ — this test's job is to LOCK the shape, not to (re-)establish it.
    const overrides = readRepoJson<Record<string, unknown>>('tests-ts/testdata/descriptor/maximal-valid-shape.overrides.json');
    const expected = readRepoJson('tests-ts/testdata/descriptor/maximal-valid-shape.expected.json');
