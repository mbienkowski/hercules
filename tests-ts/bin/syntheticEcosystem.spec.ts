...
import { readSource, write } from '../../scripts-ts/build/emit.mjs';
import type { ExtrasContext } from '../../scripts-ts/build/genExtras.mjs';
import { emitExtras } from '../../scripts-ts/build/genExtras.mjs';
import { dest } from '../../scripts-ts/build/genSerialize.mjs';
...
// target is one src/ecosystems/<name>.json file, never new TypeScript." A synthetic 7th ecosystem,
...
// (rather than writing a real file into src/ecosystems/, which would mutate the checked-out repo)
...
describe('a synthetic 7th ecosystem, built from existing vocabulary only', () => {
  it('parses, routes, serializes, and emits extras with zero .mts changes', () => {
    ...
    // descriptor, so it still ships, just under its own src/content-relative name).
