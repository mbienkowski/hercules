...
describe('the module format of the toolchain', () => {
  ...
  it('emits every domain as ESM, so every toolchain source is .mts', () => {
    ...
    expect(build.include).toEqual(['builder/**/*.mts', 'release/**/*.mts', 'metrics/**/*.mts']);

    // Each domain's own tests/ nests INSIDE it (builder/tests/, not a separate tests-ts/ sibling as
    // before the restructure), so the recursive `**` would also match .spec.ts files there — those
    // belong to tsconfig.tests.json's project, not this one, and must be excluded explicitly.
