 ...
 *
 * Every per-edition check in this directory iterates `TREES`, a hand-written literal. That makes the
 * literal itself a single point of failure with no failure mode: removing `gemini-cli` from it left
 * typecheck, lint and the entire suite green at sixteen fewer tests, because a test that never runs
 * cannot fail. The comment in `editions.ts` claiming "a sweep that opens nothing passes silently, and
 * that is the exact defect these guards exist to catch" was true of everything except itself.
 *
 * So the list is reconciled against what the builder actually ships, and every path helper is proved
 * to resolve on every edition. A guard whose target has moved must fail loudly, never quietly skip.
 */
describe('the edition list matches what ships', () => {
  ...
  it.each(TREES)('resolves the rubric and the reference skill on %s', (tree) => {
    for (const rel of [RUBRIC, REFERENCE]) {
      expect(distFile(tree, rel).length, `dist/${tree}/${rel} is empty`).toBeGreaterThan(0);
    }
  });
