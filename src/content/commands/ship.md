...
If the session's `build_complete` is not `true`: refuse — "Local build is not complete. Finish `${ns}build` first." — and stop (a spec-scoped invocation is the one exemption — see below).

Surface any `handed_off_by` / `handoff_note` from the session — the successor sees the note here.

Verify the working directory is a git repository (`git status` returning "not a git repository" → stop: "Not a git repository — run `git init` or open ${host} in your repo, then re-run."). Detect a detached HEAD (`git symbolic-ref --quiet HEAD` failing) → stop: "Detached HEAD — check out a branch (`git checkout -b {name}`), then re-run."
