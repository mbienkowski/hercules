...
Holds the registry/state builders plus the one canonical frozen path and spec. `FROZEN_TEST`/`SPEC`
bind three roles that must agree — the fixture default that freezes the path, the edit target the
tests block, and the assertions that read it back out of the block message — so one constant keeps
...
def build_project(tmp_path: Path, *, phase="build", frozen=(FROZEN_TEST,), **kw) -> Path:
    """A registry + state tree for one build session, returning the project dir: `build_project(tmp_path)`
    gives a project mid-Build with `FROZEN_TEST` frozen. Pass `phase=`/`frozen=` to vary the scenario."""
...
def fresh_hook_over_decoy_state(tmp_path: Path):
    ...
    sys.path — the setup that proves the hook front-loads its OWN directory. Yields the fresh module
    and restores sys.path / sys.modules on teardown."""
