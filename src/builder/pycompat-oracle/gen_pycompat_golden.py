...
src/builder/pyCompat.mts reproduces Python's ``str.isspace()``, ``str.splitlines()`` and
...
Usage:
    python src/builder/pycompat-oracle/gen_pycompat_golden.py [output-path]
...
DEFAULT_OUT = Path(__file__).resolve().parents[1] / "tests" / "testdata" / "pycompat-golden.json"
...
def build() -> dict[str, object]:
    return {
        "_comment": [
            "Generated from CPython by src/builder/pycompat-oracle/gen_pycompat_golden.py. The TypeScript pyCompat",
