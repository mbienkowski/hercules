#!/usr/bin/env bash
#
# Freshness gate for src/builder/tests/testdata/pycompat-golden.json.
#
# src/builder/pyCompat.mts reproduces Python's character classification, and that
...
committed_unicode="$(python -c 'import json,sys; print(json.load(open("src/builder/tests/testdata/pycompat-golden.json"))["unidataVersion"])')"
...
python src/builder/pycompat-oracle/gen_pycompat_golden.py "${fresh}"

if diff -q src/builder/tests/testdata/pycompat-golden.json "${fresh}" > /dev/null; then
...
echo >&2
diff -u --label committed src/builder/tests/testdata/pycompat-golden.json --label regenerated "${fresh}" \
  | head -40 | sed 's/^/    /' >&2
echo >&2
echo "Regenerate it and rebuild the NON_PRINTABLE table in src/builder/pyCompat.mts:" >&2
echo "    python src/builder/pycompat-oracle/gen_pycompat_golden.py" >&2
