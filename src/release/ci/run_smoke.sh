...
#
# `--config` is REQUIRED. The default config excludes `src/builder/tests/smoke/**` so the everyday suite
# stays deterministic, and that exclusion beats an explicit file path on the command line: without this
# flag every leg exits 1 with "No test files found". That is exactly what happened when the exclusion
# was introduced and this script was not updated alongside it, taking all six CI smoke legs red.
set -eo pipefail
