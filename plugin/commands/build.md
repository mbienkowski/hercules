...
Resolve the **artifact root** (`docs_root`, default `docs/`; see `CLAUDE.md § Artifact root resolution`), then read the project's registry entry in `~/.hercules/config.json` and its state file `~/.hercules/state/{slug}.json` (see `CLAUDE.md § Machine-local state`). If a session's `current_phase` is `"build"` and `current_spec` is set, show any `handed_off_by` / `handoff_note` and the latest `build_progress` checkpoint first, then offer:
...
On resume, reconcile against the filesystem: a spec in `delivered_specs` whose file still exists was interrupted before its delete — `git rm` it now; `pending_specs` drives remaining work. If the registry entry is missing but a state file exists, rebuild it from the state file. If the project has no entry or no session in `"build"`, proceed silently.
...
Read `*-business-requirements.md` and the spec files from the confirmed session. List all `*-spec-NN-*.md` files in delivery order (ascending `NN`). Summarise each spec's scope and what "done" means.

Read the session's `tier` from the state file. Complexity was scored once in Discover — **do not re-classify, re-derive, escalate, or de-escalate it.** The tier scales delivery depth (review, advisor involvement); only a manual user override changes it.

If the session has `pending_specs` from a prior run, use that order; otherwise derive it from a shell-sort of spec filenames.

### Step 4 — Present the delivery plan

Show the delivery plan for review: each spec, the requirement(s) it satisfies (its `satisfies:` links), the delivery order, and how the specs are grouped. State the count: `"{N} specs in delivery order."` Invite the user to shape it before any code is written — they can **re-batch** ("do spec 1 & 2 together"), **reorder**, or set the **cadence**: deliver all in one pass, or ship each spec as it lands.
...
3. **Write failing tests.** Invoke `write-test-scenarios` for the spec's scope and linked design section. Gate: every new test compiles **and fails for the right reason** (implementation missing — not a syntax or runner error). Frontend scope: suggest Gherkin e2e scenarios (Cypress / Playwright); keep scenario files in source control. Record the test files to `frozen_test_files`. **The tests are now frozen** — the agent does not edit them. Enforced, not promised: before the advance step, `git diff` the frozen files; any change outside the sanctioned user-decision path (step 5) blocks.
...
Spawn `hercules:cynical-reviewer` (Spec Sync) to cross-check the whole delivery — *does what we built match what we set out to build?* The specs are retired, so it reads each spec's record from the `build_progress` checkpoint + the permanent `*-business-requirements.md`, not the deleted files. Per spec: an intentional improvement is documented; a scope reduction is marked deferred; a bug or regression is a **blocker**. Then requirement traceability & drift, with cited evidence: every requirement maps to a delivered spec and a named passing test (`✓ [requirement] → evidence` / `✗ [requirement] → NOT COVERED`); and the reverse — any shipped behaviour with no originating requirement is scope drift, surfaced for a disposition (amend `*-business-requirements.md`, or annotate as deliberate/trivial). Drift on a high-risk surface (auth, secrets, money, migration, deletion, prod-config, concurrency) **blocks** until requirement-backed. Scale depth to tier: trivial/low get a light pass; medium+ the full review; high/critical also spawn the relevant domain-expert agent.
