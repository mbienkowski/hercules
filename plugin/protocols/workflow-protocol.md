...
Ship. Steps run in listed order; none is reordered or skipped (a registry row may exempt a step
for its stated mode); a GATE halts the phase until met.
...
Class: hook = harness-enforced; executable = deterministic check; state-checkable = state read;
prompt-only = discipline.

| ID | Phase · step | Scope | Rule | Class |
|---|---|---|---|---|
| G1 | build · write-failing-tests | span → retire spec | Frozen tests are never edited during implementation (a PreToolUse hook blocks it); only exit: the round-limit user decision. | hook |
...
| G3 | build · scaffold | span → implement | Scaffold compiles first; tests fail because implementation is missing; no logic until then. | prompt-only |
| G4 | build · traceability | step | Each satisfies-linked requirement and acceptance criterion maps to a named passing test before retire. | executable |
...
| G6 | ship · preconditions | phase | Session Ship gates on build_complete; a spec-scoped ship skips only that gate, stages only the current spec's files, omits the PR, and writes no session field (current_phase, build_complete, shipped_commit, shipped_pr). | state-checkable |
...
Prepended above the A2A Agent-Injected Core in every workflow spawn; copy every registry row
...
Context: {code-of-conduct.md} + {artifact §-sections — the minimal slice, never a whole
...
| Role | Expected (injected verbatim) | Slice (design.md spec-template sections) |
|---|---|---|
