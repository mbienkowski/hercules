...
- **Spec-sync (mandatory last step).** After findings resolve, update the spec to match what actually shipped — as a present-tense snapshot, never a changelog. Intentional improvement → update; scope reduction → mark deferred with reason; missing work → raise as Blocker, never paper over.
