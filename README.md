...
- **Models** — the Hercules persona **defaults to `opus`**, the tier best suited to its orchestration
  work. That default is just an initial selection, not a lock: **switch to any model anytime with
  `/model`** (or set your own default in session/project settings) and Hercules follows it. Some
  advisor agents pin smaller models (`sonnet`, `haiku`) to keep debates cheap.
