...
def generate_plugin_js(
    ...
    for name, metadata, prompt in commands:
        command_entries[f"hercules:{name}"] = {
            "description": metadata.get("description", ""),
...
def main() -> int:
    ...
    def process_agent(name: str, is_default: bool) -> None:
        ...
        # Model IDs are provider-specific in OpenCode; leave them unset so the user's configured
        # default model is used. They can override per-agent in their own opencode.json.
    ...
    for name in commands:
        ...
        # Leave model unset; OpenCode uses the user's default model for commands.
        metadata.pop("model", None)
        transformed_body = transform_content(body)
