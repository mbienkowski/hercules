"""Grok Build target: a Claude-Code-shaped plugin — Grok Build reads Claude-format plugins natively.

Ships Grok's own ``PreToolUse`` hook wiring (rooted at ``${GROK_PLUGIN_ROOT}``) plus the canonical
frozen-test guard byte-copied from the shared source; capability prose lives in
``src/targets/grok-build/CAPABILITIES.md`` (copied verbatim by ``emit_shared``).
"""
