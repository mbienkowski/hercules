...
# INSTALL_FLAGS, CLI, and CHANNEL (pinned = ci.yml default, latest = nightly).
#
...
# executed — never streamed straight to bash; there is no pinned checksum for a rolling installer, so
# main-only gating (see smoke_matrix.py) is the documented supply-chain compromise. Ends in
# `$CLI --version`, so an absent binary fails the job.
set -euo pipefail
