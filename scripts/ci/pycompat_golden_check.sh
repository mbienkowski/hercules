...
#
# scripts-ts/build/pyCompat.mts reproduces Python's character classification, and that
...
python scripts/ci/gen_pycompat_golden.py "${fresh}"
...
diff -u --label committed tests/testdata/pycompat-golden.json --label regenerated "${fresh}" \
  | head -40 | sed 's/^/    /' >&2
echo >&2
echo "Regenerate it and rebuild the NON_PRINTABLE table in scripts-ts/build/pyCompat.mts:" >&2
echo "    python scripts/ci/gen_pycompat_golden.py" >&2
