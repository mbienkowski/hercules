...
3. **Scan** — `python3 ${CLAUDE_PLUGIN_ROOT}/tools/coc_scan.py all --root <root> --contract 1`.
...
7. **Gate & present** — the gate is a program, not a promise.
   ...
   - Pipe it to `python3 ${CLAUDE_PLUGIN_ROOT}/tools/coc_audit.py draft --contract 1`.
...
- **Read it mechanically first:** `python3 ${CLAUDE_PLUGIN_ROOT}/tools/coc_audit.py existing --contract 1
