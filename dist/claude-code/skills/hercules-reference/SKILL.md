...
truth, and `python3 ${CLAUDE_PLUGIN_ROOT}/tools/registry_sync.py repair plan|apply [--confirm]` heals the pointer half of a torn two-file write (when a `state_file` pointer names a file that does not exist, but the conventional `{slug}.json` does). Orphaned state files with no registry entry are reported but never fabricated. The **delivery state**
