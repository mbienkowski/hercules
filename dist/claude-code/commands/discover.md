...
Write the session-init state under `~/.hercules/` (see `hercules-reference § Machine-local state`), never the repo. First, create or update the registry entry if needed (`directory`, `docs_root`, `state_file`), preserving `repositories`, `frozen_hook`, `keep_specs` on existing entries. Then run `python3 ${CLAUDE_PLUGIN_ROOT}/tools/state_patch.py apply --project-slug {slug} --session-id {new-session-id} --set active_session={new-session-id} --set current_phase=discover --set tier={tier} --set tier_rationale={rationale} --confirm` to write the state file's session atomically. Non-zero exit: relay the output and stop. Preserve other entries/sessions.
...
(`python3 ${CLAUDE_PLUGIN_ROOT}/tools/doc_report.py rubric` prints them; `… template --kind
