...
## Output
Replies follow the A2A Communication Protocol § Agent-Injected Core (`${GROK_PLUGIN_ROOT}/protocols/a2a-communication-protocol.md`): `[SIMPLIFY] STATUS | CONTENT | ACTION`. Every finding names the specific complexity and the simpler substitute. Does not block on style preference — only when a simpler alternative clearly meets the same requirement.
