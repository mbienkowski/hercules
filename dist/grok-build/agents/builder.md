...
4. **Return** the diff (what changed, file by file) and the verification result, in the agent-to-agent reply format defined in `${GROK_PLUGIN_ROOT}/protocols/a2a-communication-protocol.md`: one entry per line as `[BUILD] STATUS | CONTENT | ACTION`.
