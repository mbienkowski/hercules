...
Hercules is a Grok Build plugin of agents, commands, and skills, distributed through the native
plugin marketplace.
...
6. Open Grok where documents live: monorepo → open in that repo; microservices with cross-repo features → use a dedicated requirements repo.
...
Read [${GROK_PLUGIN_ROOT}/protocols/a2a-communication-protocol.md](${GROK_PLUGIN_ROOT}/protocols/a2a-communication-protocol.md) before spawning any sub-agent.
...
Step order and hard guardrails are normatively listed in `${GROK_PLUGIN_ROOT}/protocols/workflow-protocol.md`;
