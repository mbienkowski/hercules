...
**4. Record.** Session-wide ship only — spec-scoped skips this. Run `python3 ${GROK_PLUGIN_ROOT}/tools/state_patch.py apply --project-slug {slug} --session-id {id} --set current_phase=shipped --set build_complete=false --set shipped_commit={sha} --set shipped_pr={url-if-available} --confirm`. Non-zero exit: relay the output and stop. Show `"Shipped {session-slug} at {short-SHA}."`
