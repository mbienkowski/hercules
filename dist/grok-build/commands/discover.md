...
Paraphrase what you heard in 2–3 sentences so the user can correct any misunderstanding before scoring. Score against `${GROK_PLUGIN_ROOT}/protocols/debate-consensus-protocol.md` § complexity — both signal columns, then the higher of the two — and never restate its numbers here. Then show the judgement together with the choices open to the user:
...
(`python3 ${GROK_PLUGIN_ROOT}/tools/doc_report.py rubric` prints them; `… template --kind
...
here to remember. Then say: "The requirements are locked. Ready for **Design**? Run `/hercules:design` — we'll shape the solution and delivery sequence there."
