...
the launch directory; if Grok was opened away from the code or several candidate roots exist, list
...
3. **Scan** — run `python3 ${GROK_PLUGIN_ROOT}/tools/coc_scan.py all --root <root> --contract 1` for the
...
7. **Gate & present** — the gate is a program, not a promise. Build the envelope — every drafted rule
   ...
   **§ Rules envelope**) — and pipe it to
   `python3 ${GROK_PLUGIN_ROOT}/tools/coc_audit.py draft --contract 1`. Exit 0 ships; any other exit means
...
`python3 ${GROK_PLUGIN_ROOT}/tools/coc_audit.py existing --contract 1 --file <coc> --root <root>` reports
