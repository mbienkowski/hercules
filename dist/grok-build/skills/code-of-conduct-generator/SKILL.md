...
- Resolve the **target** repo per `hercules-reference § Code-of-conduct resolution`, not the launch
  directory. If Grok was opened away from the code, or several roots are candidates, list them
...
3. **Scan** — `python3 ${GROK_PLUGIN_ROOT}/tools/code_of_conduct/coc_scan.py all --root <root> --contract 2`.
...
3b. **Extract the architecture** — per **§ Architecture extractor**. Write a small read-only
   ...
   `python3 ${GROK_PLUGIN_ROOT}/tools/code_of_conduct/coc_scan.py extract --root <root> --extractor <script> --contract 2`.
...
7. **Gate & present** — the gate is a program, not a promise.
   - Build the envelope: each rule as `{id, section, tag, text, check, citations}` plus the `facts`,
     ...
     (**§ Rules envelope**).
   - Pipe it to `python3 ${GROK_PLUGIN_ROOT}/tools/code_of_conduct/coc_gate.py draft --contract 2`.
   ...
   - Lay the rules out as markdown, then `python3 ${GROK_PLUGIN_ROOT}/tools/code_of_conduct/coc_lint.py
...
- **Read it mechanically first:** `python3 ${GROK_PLUGIN_ROOT}/tools/code_of_conduct/coc_lint.py
