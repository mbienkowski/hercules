...
**Plan mode — required.** Call `plan mode` at the start. Every draft is a full inline proposal. Iterate freely; always regenerate the complete draft — never patch sections; never skip steps. At the **Plan approval** gate, on the user's approval, call `approval` (`auto`), then write.

## Step 0 — Artifact location & prior context

Resolve the **artifact root** per `AGENTS.md § Artifact root resolution`: a `code-of-conduct.md`
...
On the user's approval, call `approval` (`auto`), then write (Step 7) — no further prompts.
...
Write the session-init state under `~/.hercules/` (see `AGENTS.md § Machine-local state`), never the
