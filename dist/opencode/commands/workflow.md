...
**Plan mode — required across all four phases.** Open each phase with `plan mode`; present a full inline proposal; at the **Plan approval** gate, on the user's "approved", call `approval` (`auto`) — the phase then writes or executes without further prompts — then `plan mode` for the next phase. Iterate freely; write only on approval. Never patch sections — always regenerate the complete draft.
