...
7. **Traceability.** An **independent review** (`hercules-reference § Independent review`), never a self-check: spawn `cynical-reviewer` with the packet (`protocols/workflow-protocol.md#packet`); reading the spec's `satisfies:` + `*-business-requirements.md` + the test files **directly**, it maps each `satisfies:` requirement §section to ≥1 **named passing test** — and each acceptance criterion to a test. Where the spec carries `covers:`, map by **scenario identifier**: each one must appear in the name of a passing test, found by searching for it rather than by judging whether two sentences agree. Synthesise: an uncovered requirement → stop, re-enter `/hercules:design`.
...
Ask: "Anyone taking over? Say 'handoff to {name}: {note}', or skip." Then run `python3 tools/state_patch.py apply --project-slug {slug} --session-id {id} --set handed_off_by={handoff-name} --set handoff_note={handoff-note} --set current_spec=null --set pending_specs=[] --set build_complete=true --unset current_spec_round --confirm` to write all end-of-Build mutations atomically; Non-zero exit: relay the output and stop.
