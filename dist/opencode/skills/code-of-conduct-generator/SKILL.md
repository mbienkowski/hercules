...
3. **Scan** — run `python3 tools/coc_scan.py all --root <root> --contract 1` for the
...
7. **Gate & present** — the gate is a program, not a promise. Build the envelope — every drafted rule
   ...
   **§ Rules envelope**) — and pipe it to
   `python3 tools/coc_audit.py draft --contract 1`. Exit 0 ships; any other exit means
...
`python3 tools/coc_audit.py existing --contract 1 --file <coc> --root <root>` reports
