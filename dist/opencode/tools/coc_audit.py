...
the directive budget. Two properties are not negotiable. It is a PURE function of its input: no path
is opened and no subprocess runs, so a hostile repository has no surface here. And it fails CLOSED —
any unclassified error is a refusal, never a pass, because a gate that errs open is not a gate.
"""
