...
Section depth follows the tier — `python3 ${CURSOR_PLUGIN_ROOT}/tools/doc_lint.py template --kind spec
...
(`python3 ${CURSOR_PLUGIN_ROOT}/tools/doc_report.py rubric`), writing its answers beside the spec as
...
nothing here to remember. Then say: "The specs and delivery sequence are locked. Ready to **Build**? Run `/build` — I'll present a delivery plan first, then deliver the specs."
