...
**Frozen tests on Cursor are advisory in the IDE (Cursor cannot block a file edit).** If the agent edits a frozen test in Composer, the hook surfaces a notice and does **not** revert — you undo it (Ctrl+Z) or grant an override (`"change test X — <why>"`). The hard teeth are elsewhere and unaffected: `beforeShellExecution`/`beforeMCPExecution` deny a frozen write/commit, and Step 10's acceptance backstop re-hashes every frozen test before retiring a spec, so a tampered test can never ship regardless of the edit path. **If `python3` is not on PATH**, these hooks are inactive (fail-open) — say so at Build start, since frozen-test protection then rests on the acceptance backstop alone until `python3` is available.
