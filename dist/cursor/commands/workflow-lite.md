...
Run `/discover` (Steps 0–7). For speed: cap advisor debate at **1 round** (low-complexity mode — see `protocols/debate-consensus-protocol.md`). Skip the full debate for `trivial` tasks; run 1 round for `low`/`medium`/`high`/`critical`.
...
Run `/design` (Steps 1–9). Read the requirements. Debate capped at **1 round max**. For `trivial`/`low` tiers, skip the independent reviewer spawn (coverage gate is self-checked); for `medium+`, make the reviewer optional — ask: "Run independent review? (yes/no)". On "no", proceed with a brief self-check note.
...
Run `/build`. Same delivery plan, same frozen-test rules (`protocols/workflow-protocol.md`), same quality gates. Per-spec spawn uses the delegation packet (`protocols/workflow-protocol.md#packet`) but with shorter prompts (no re-explaining full protocol each time — reference by section).
...
Otherwise run `/ship`. When done:

> ✓ Shipped. Commit: [one-liner]. Run `/workflow-lite` for the next feature.
