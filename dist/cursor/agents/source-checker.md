...
## Output
Replies follow the A2A Communication Protocol § Agent-Injected Core (`${CURSOR_PLUGIN_ROOT}/protocols/a2a-communication-protocol.md`): `[SOURCE-CHECK] STATUS | CONTENT | ACTION`. Do not block on a claim that needs external research — flag it with its risk and suggested verification path, and let the human decide.
