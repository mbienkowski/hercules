...
Hercules is a Gemini CLI plugin of agents, commands, and skills, distributed through the native
plugin marketplace.
...
6. Open Gemini where documents live: monorepo → open in that repo; microservices with cross-repo features → use a dedicated requirements repo.
...
Read [${extensionPath}/protocols/a2a-communication-protocol.md](${extensionPath}/protocols/a2a-communication-protocol.md) before spawning any sub-agent.
...
Step order and hard guardrails are normatively listed in `${extensionPath}/protocols/workflow-protocol.md`;
