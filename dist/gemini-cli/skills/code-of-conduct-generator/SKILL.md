...
3b. **Extract the architecture** — per **§ Architecture extractor**. Write a small read-only
   ...
   `python3 ${extensionPath}/tools/code_of_conduct/coc_scan.py extract --root <root> --extractor <script> --contract 2`.
...
8. **Approve & write** — on approval: leave plan mode → write atomically (temp + rename).
   ...
   for `GEMINI.md` in chat, naming the resolved path; adding it is the user's edit.
