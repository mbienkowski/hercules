...
- Resolve the **target** repo per `hercules-reference § Code-of-conduct resolution`, not the launch
  directory. If Gemini was opened away from the code, or several roots are candidates, list them
...
3. **Scan** — `python3 ${extensionPath}/tools/coc_scan.py all --root <root> --contract 1`.
...
7. **Gate & present** — the gate is a program, not a promise.
   ...
   - Pipe it to `python3 ${extensionPath}/tools/coc_audit.py draft --contract 1`.
...
- **Read it mechanically first:** `python3 ${extensionPath}/tools/coc_audit.py existing --contract 1
