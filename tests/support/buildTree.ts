...
/**
 * Every file under `root` whose name ends with one of `extensions` (default `['.md']`), skipping any
 * directory named in `skipDirs`. Absolute paths, sorted.
 */
export function pathsUnder(
  root: string,
  { extensions = ['.md'], skipDirs = [] as readonly string[] }: { extensions?: readonly string[]; skipDirs?: readonly string[] } = {},
): string[] {
