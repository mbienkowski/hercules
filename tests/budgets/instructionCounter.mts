/**
 * Atomic instruction counting, feeding the loading-chain ceilings in `loadingChains.mts`. A UNIT is a
 * bullet, numbered item or bold-labelled block, fenced content included; it splits on `,` `;` `&`
 * `and` `or` `then`, and every fragment of two or more words is one directive. It deliberately
 * OVER-counts — undercounting a chain's cognitive load is the dangerous direction.
 */
...
/**
 * Every unit line in `text`, matched regardless of fence state — a fence delimiter starts with three
 * backticks and can never satisfy a unit pattern, and a numbered rule inside a fence still counts.
 */
