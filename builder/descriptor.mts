...
// as source (Vitest, one directory level under the repo root) and as compiled output
// (`.ts-out/builder/descriptor.mjs`, two levels under it, since `tsc`'s `rootDir: "."` preserves the
// full domain path rather than stripping a `scripts-ts/`-style prefix as the old layout did) — a
// fixed relative-hop count cannot be correct in both. Every real entry point (the Makefile's
// targets, the npm scripts, Stryker's sandboxed runner — see tests/support/repo.ts's own repoRoot
// for the fuller reasoning) already runs from the repo root, so `process.cwd()` resolves correctly
// in both execution modes without needing to track either one's directory depth.
...
export const ECOSYSTEMS_DIR = join(REPO_ROOT, 'ecosystems');
