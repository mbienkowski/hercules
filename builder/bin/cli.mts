...
// (Vitest, two directory levels under the repo root) and as compiled output
// (`.ts-out/builder/bin/cli.mjs`, three levels under it, since `tsc`'s `rootDir: "."` preserves the
// full domain path), so a fixed relative-hop count cannot be correct in both. See
// descriptor.mts's own REPO_ROOT for the fuller reasoning — every real entry point already runs
// from the repo root.
const REPO_ROOT = process.cwd();
const SRC_CONTENT = join(REPO_ROOT, 'content');
const DIST = join(REPO_ROOT, 'dist');
// The canonical frozen-test guard + the one generic write-gate adapter live in the NEUTRAL
// hooks/ tree; every ecosystem ships byte-copies, so the write-gate logic has one source of truth.
const SHARED_HOOKS_SRC = join(REPO_ROOT, 'hooks');
