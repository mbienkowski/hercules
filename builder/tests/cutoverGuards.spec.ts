...
describe('no source file hardcodes a path into the retired plugin/ folder', () => {
  it('scans every Python test and tooling file (other than conftest.py)', () => {
    ...
    const files = [
      ...globSync('tests/repo/**/*.py', { cwd: repoRoot }),
      ...globSync('hooks/tests/**/*.py', { cwd: repoRoot }),
      ...globSync('release/**/*.py', { cwd: repoRoot }),
      ...globSync('builder/pycompat-oracle/**/*.py', { cwd: repoRoot }),
...
it('mutation testing targets the current hooks location, not the retired one', () => {
  ...
  expect(pyproject).toContain('paths_to_mutate = "hooks/"');
  expect(pyproject).not.toContain('src/hooks/');
