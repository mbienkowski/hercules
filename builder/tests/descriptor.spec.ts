...
describe('accepting the maximal valid shape', () => {
  it('parses every field generator, route kind, and template value kind into the EXACT expected shape', () => {
    ...
    const overrides = readRepoJson<Record<string, unknown>>('builder/tests/testdata/descriptor/maximal-valid-shape.overrides.json');
    const expected = readRepoJson('builder/tests/testdata/descriptor/maximal-valid-shape.expected.json');
