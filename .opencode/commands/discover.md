...
**Plan mode — required.** Call **Begin OpenCode plan mode** (present the complete proposal before any write; edits stay gated until explicit approval) at the start. Every draft is a full inline proposal. Iterate freely; always regenerate the complete draft — never patch sections; never skip steps. At the **Plan approval** gate, on the user's approval, call **Exit OpenCode plan mode after explicit user approval**, then execute (`auto`), then write.
...
On the user's approval, call **Exit OpenCode plan mode after explicit user approval**, then execute (`auto`), then write (Step 7) — no further prompts.
