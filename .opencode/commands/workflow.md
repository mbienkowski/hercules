...
Guided end-to-end delivery: all four phases in one flow. Plugin-file citations (`CLAUDE.md §…`, `protocols/…`) live in this plugin's directory — the parent of the folder holding this command file, not the user's repo; search the plugin dir if needed.

**Plan mode — required across all four phases.** Open each phase with **Begin OpenCode plan mode** (present the complete proposal before any write; edits stay gated until explicit approval); present a full inline proposal; at the **Plan approval** gate, on the user's "approved", call **Exit OpenCode plan mode after explicit user approval**, then execute (`auto`) — the phase then writes or executes without further prompts — then **Begin OpenCode plan mode** (present the complete proposal before any write; edits stay gated until explicit approval) for the next phase. Iterate freely; write only on approval. Never patch sections — always regenerate the complete draft.
