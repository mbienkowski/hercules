...
**Plan mode — required.** Build opens in plan mode: call **Begin OpenCode plan mode** (present the complete proposal before any write; edits stay gated until explicit approval), present the **delivery plan**, and exit through the **Plan approval** gate below. Execution then runs automatically, spec by spec (a *ship each* "ship now" opens Ship's own plan).
