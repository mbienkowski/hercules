...
Stage, commit, and optionally push the delivered work. Plugin-file citations (`CLAUDE.md §…`, `protocols/…`) live in this plugin's directory — the parent of the folder holding this command file, not the user's repo; search the plugin dir if needed.

**Plan mode — required.** Call **Begin OpenCode plan mode** (present the complete proposal before any write; edits stay gated until explicit approval); present a complete Ship plan; at the **Plan approval** gate — *you approve the phase after reviewing the plan* — when the user says **"approved"** or clicks **Accept**, call **Exit OpenCode plan mode after explicit user approval**, then execute (`auto`), then execute all steps automatically — no further questions.

---

## Precondition check (before beginning OpenCode plan mode)
...
Verify the working directory is a git repository (`git status` returning "not a git repository" → stop: "Not a git repository — run `git init` or open OpenCode in your repo, then re-run."). Detect a detached HEAD (`git symbolic-ref --quiet HEAD` failing) → stop: "Detached HEAD — check out a branch (`git checkout -b {name}`), then re-run."
...
## Execution (after exiting OpenCode plan mode — automatic, no further prompts)
