...
**Begin OpenCode plan mode** (present the complete proposal before any write; edits stay gated until explicit approval), closed at the single **Plan approval** gate with **Exit OpenCode plan mode after explicit user approval**, then execute (`auto`), after which
