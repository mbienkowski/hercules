...
1. **Plan mode & mode** — call **Begin OpenCode plan mode** (present the complete proposal before any write; edits stay gated until explicit approval) first, before any scanning; give a chat summary of the
...
8. **Approve & write** — on approval: **Exit OpenCode plan mode after explicit user approval**, then execute (`auto`) → write atomically (temp + rename) → add a
